...
const apple = read("services/appleAuth.ts");
...
assert.ok(apple.includes("Crypto.CryptoDigestAlgorithm.SHA256"), "Apple não recebe nonce em SHA-256");
assert.ok(apple.includes("nonce: hashedNonce"), "Apple ainda recebe o nonce bruto");
assert.ok(apple.includes("nonce: rawNonce"), "Supabase não recebe o nonce bruto para validação");
assert.ok(apple.includes("state: expectedState"), "Apple OAuth state ausente");
assert.ok(apple.includes("credential.state !== expectedState"), "Apple OAuth state não é validado");
assert.ok(diagnostics.includes("[auth-diagnostic]"), "Diagnóstico seguro de autenticação ausente");
assert.ok(!diagnostics.includes("password:"), "Diagnóstico não pode registrar senha");
...
console.log("Auth UI e Apple: verificações aprovadas.");
