...
function callbackCode(callbackUrl: string) {
  ...
  const oauthError = url.searchParams.get("error") ?? fragmentParams.get("error");
...
export async function completeGoogleOAuthCode(code: string): Promise<GoogleAuthResult> {
  ...
  const promise = (async () => {
    ...
    let stage = "supabase_code_exchange";
    try {
...
export async function signInWithGoogle(): Promise<GoogleAuthResult> {
  ...
  let started;
  try {
    started = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
    ...
    logAuthFailure("google", "oauth_start", error);
    throw new GoogleAuthError(
      "provider_unavailable",
  ...
  if (started.error || !started.data.url) {
    logAuthFailure("google", "oauth_start", started.error);
    throw new GoogleAuthError(
      "provider_unavailable",
      "O login com Google não está disponível agora. Tente novamente mais tarde.",
    );
  ...
  try {
    ...
    logAuthFailure("google", "browser_authorization", error);
    throw new GoogleAuthError("provider_unavailable", "O Google não concluiu a autenticação.");
  } finally {
    try { WebBrowser.dismissAuthSession(); } catch { /* A sessão já pode estar encerrada. */ }
  }
