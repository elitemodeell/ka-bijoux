...
export async function signInWithApple(): Promise<AppleAuthResult> {
  ...
  let stage = "native_authorization";
  ...
  try {
    ...
    if (credential.state !== expectedState) {
      throw new AppleAuthError("invalid_credential", "O retorno da Apple não corresponde a esta tentativa de acesso.");
    }
    ...
    if (signedIn.error || !signedIn.data.session) {
      throw signedIn.error ?? new AppleAuthError("invalid_credential", "Não foi possível validar a credencial da Apple.");
    }
    ...
    if (stage === "supabase_token_exchange") {
