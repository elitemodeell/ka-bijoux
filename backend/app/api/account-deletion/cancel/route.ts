...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await cancelAccountDeletion(input.token);
    if (result.status === "invalid") {
      return apiError("Este link é inválido, expirou ou já foi utilizado.", 410);
    }
    return apiSuccess({ message: "A solicitação foi cancelada. Sua conta permanece ativa." });
  } catch (error) {
    if (error instanceof z.ZodError) return apiError("Solicitação inválida.", 422);
    console.error("account-deletion cancellation failed", error);
    return apiError("Não foi possível cancelar a solicitação.", 500);
  }
}
