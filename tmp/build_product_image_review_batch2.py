...
SOURCE_DIR = Path.home() / "Downloads" / "produtodos atualizados 2"
OUT_DIR = ROOT / "reports" / "product-image-review-batch-2"
...
def build_contact_sheets(items: list[dict]) -> None:
    ...
    for sheet_idx, start in enumerate(range(0, len(items), cols * rows), start=1):
        ...
        draw.text((pad, 12), f"KA Bijoux - lote 2 - folha {sheet_idx}", fill=(35, 29, 32), font=font_title)
        ...
        for idx, item in enumerate(sheet_items):
            ...
            for line_no, line in enumerate(wrap_text(item["name"], font_small, thumb_w - 4)):
                draw.text((x, label_y + 26 + line_no * 17), line, fill=(55, 48, 52), font=font_small)
